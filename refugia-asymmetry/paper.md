...
Our framework extends the refugia habitat metric approach of Lee et al. (2025) from a single-pair diagnostic tool to a multi-pair, multi-continent predictive framework. By testing ten pairs spanning all four quadrants of the thermal asymmetry matrix across five continents, we show that the direction and severity of refugia change is predictable from the relative thermal niche positions of the interacting species, transforming a case-specific diagnostic into a general screening principle.
...
Lee, F., Kusabs, I.A.K., Perry, G.L.W. & MacNeil, C. (2025) Defining climate change refugia for native freshwater crayfish using species distribution modelling: incorporating invasive species. *Web Ecology*, 25, 1--15.
